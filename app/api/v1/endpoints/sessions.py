...
async def post_summary(session_id: str):
    ...
    supabase_client.upsert_session_summary(session_id, summary_md, tags)
    transcript_rows = supabase_client.get_transcripts(session_id) or [
        {"timestamp_iso": r.get("timestamp_iso", ""),
         "speaker": r.get("speaker", ""),
         "text": r.get("text", "")}
        for r in records
    ]
    report_url = supabase_client.export_report(session_id, summary_md, tags, transcript_rows)
...
async def get_metrics_csv():
    return PlainTextResponse(eval_metrics.all_csv(), media_type="text/csv")
