...
Admin role
----------
There is no separate admin-role table. An admin is simply a Supabase Auth
user whose user_metadata contains {"role": "admin"} — set this manually via:

    supabase_auth._get_admin().auth.admin.update_user_by_id(
        user_id, {"user_metadata": {"role": "admin"}}
    )

_user_to_dict() surfaces this as `isAdmin: bool`, checked by
app.api.deps.require_admin.
"""
